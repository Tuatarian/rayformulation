...
template RAYLIB_VERSION*(): auto = "4.0-dev"
...
proc GetTouchPointCount*(): int32 {.RLAPI, importc: "GetTouchPointCount".} # Get touch points count
...
proc DrawLineStrip*(points: ptr Vector2; pointCount: int32; color: Color) {.RLAPI, importc: "DrawLineStrip".} # Draw lines sequence
...
proc DrawTriangleFan*(points: ptr Vector2; pointCount: int32; color: Color) {.RLAPI, importc: "DrawTriangleFan".} # Draw a triangle fan defined by points (first vertex is the center)
proc DrawTriangleStrip*(points: ptr Vector2; pointCount: int32; color: Color) {.RLAPI, importc: "DrawTriangleStrip".} # Draw a triangle strip defined by points
...
proc LoadImagePalette*(image: Image; maxPaletteSize: int32; colorCount: pointer): ptr Color {.RLAPI, importc: "LoadImagePalette".} # Load colors palette from image as a Color array (RGBA - 32bit)
...
proc DrawTexturePoly*(texture: Texture2D; center: Vector2; points: ptr Vector2; texcoords: ptr Vector2; pointCount: int32; tint: Color) {.RLAPI, importc: "DrawTexturePoly".} # Draw a textured polygon
...
proc GetColor*(hexValue: uint32): Color {.RLAPI, importc: "GetColor".} # Get Color structure from hexadecimal value
...
proc LoadFontEx*(fileName: cstring; fontSize: int32; fontChars: pointer; glyphCount: int32): Font {.RLAPI, importc: "LoadFontEx".} # Load font from file with extended parameters
...
proc LoadFontFromMemory*(fileType: cstring; fileData: UncheckedArray[byte]; dataSize: int32; fontSize: int32; fontChars: pointer; glyphCount: int32): Font {.RLAPI, importc: "LoadFontFromMemory".} # Load font from memory buffer, fileType refers to extension: i.e. '.ttf'
proc LoadFontData*(fileData: UncheckedArray[byte]; dataSize: int32; fontSize: int32; fontChars: pointer; glyphCount: int32; typex: int32): ptr GlyphInfo {.RLAPI, importc: "LoadFontData".} # Load font data for further use
proc GenImageFontAtlas*(chars: ptr ptr GlyphInfo; recs: ptr Rectangle; glyphCount: int32; fontSize: int32; padding: int32; packMethod: int32): Image {.RLAPI, importc: "GenImageFontAtlas".} # Generate image font atlas using chars info
proc UnloadFontData*(chars: ptr GlyphInfo; glyphCount: int32) {.RLAPI, importc: "UnloadFontData".} # Unload font chars info data (RAM)
...
proc DrawTextPro*(font: Font; text: cstring; position: Vector2; origin: Vector2; rotation: float32; fontSize: float32; spacing: float32; tint: Color) {.RLAPI, importc: "DrawTextPro".} # Draw text using Font and pro parameters (rotation)
...
proc GetGlyphIndex*(font: Font; codepoint: int32): int32 {.RLAPI, importc: "GetGlyphIndex".} # Get glyph index position in font for a codepoint (unicode character), fallback to '?' if not found
proc GetGlyphInfo*(font: Font; codepoint: int32): GlyphInfo {.RLAPI, importc: "GetGlyphInfo".} # Get glyph font info data for a codepoint (unicode character), fallback to '?' if not found
proc GetGlyphAtlasRec*(font: Font; codepoint: int32): Rectangle {.RLAPI, importc: "GetGlyphAtlasRec".} # Get glyph rectangle in font atlas for a codepoint (unicode character), fallback to '?' if not found
# Text codepoints management functions (unicode characters)
proc LoadCodepoints*(text: cstring; count: pointer): pointer {.RLAPI, importc: "LoadCodepoints".} # Load all codepoints from a UTF-8 text string, codepoints count returned by parameter
proc UnloadCodepoints*(codepoints: pointer) {.RLAPI, importc: "UnloadCodepoints".} # Unload codepoints data from memory
proc GetCodepointCount*(text: cstring): int32 {.RLAPI, importc: "GetCodepointCount".} # Get total number of codepoints in a UTF-8 encoded string
proc GetCodepoint*(text: cstring; bytesProcessed: pointer): int32 {.RLAPI, importc: "GetCodepoint".} # Get next codepoint in a UTF-8 encoded string, 0x3f('?') is returned on failure
proc CodepointToUTF8*(codepoint: int32; byteSize: pointer): cstring {.RLAPI, importc: "CodepointToUTF8".} # Encode one codepoint into UTF-8 byte array (array length returned as parameter)
proc TextCodepointsToUTF8*(codepoints: pointer; length: int32): ptr char {.RLAPI, importc: "TextCodepointsToUTF8".} # Encode text as codepoints array into UTF-8 text string (WARNING: memory must be freed!)
...
proc TextReplace*(text: ptr char; replace: cstring; by: cstring): ptr char {.RLAPI, importc: "TextReplace".} # Replace text string (WARNING: memory must be freed!)
proc TextInsert*(text: cstring; insert: cstring; position: int32): ptr char {.RLAPI, importc: "TextInsert".} # Insert text in a position (WARNING: memory must be freed!)
...
proc DrawTriangleStrip3D*(points: ptr Vector3; pointCount: int32; color: Color) {.RLAPI, importc: "DrawTriangleStrip3D".} # Draw a triangle strip defined by points
...
proc GetModelBoundingBox*(model: Model): BoundingBox {.RLAPI, importc: "GetModelBoundingBox".} # Compute model bounding box limits (considers all meshes)
...
proc GenMeshCone*(radius: float32; height: float32; slices: int32): Mesh {.RLAPI, importc: "GenMeshCone".} # Generate cone/pyramid mesh
...
proc LoadModelAnimations*(fileName: cstring; animCount: pointer): ptr ModelAnimation {.RLAPI, importc: "LoadModelAnimations".} # Load model animations from file
...
proc UpdateSound*(sound: Sound; data: pointer; sampleCount: int32) {.RLAPI, importc: "UpdateSound".} # Update sound buffer with new data
...
proc UpdateAudioStream*(stream: AudioStream; data: pointer; frameCount: int32) {.RLAPI, importc: "UpdateAudioStream".} # Update audio stream buffers with data
